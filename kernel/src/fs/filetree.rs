...
use crate::{sync::mutex::Mutex, device::BLK_CONTROL, memory::{page::{PAGE_ALLOCATOR, alloc}, addr::{PhysAddr, PAGE_SIZE, get_buf_from_phys_page}}, runtime_err::RuntimeError};
...
impl FileTreeNode {
    ...
    pub fn create(&mut self, filename: &str) -> Result<(), RuntimeError> {
        ...
        // 分割文件路径
        let location: Vec<&str> = filename.split("/").collect();

        let mut tree_node = if filename.chars().nth(0).unwrap() == '/' {
            open("/")?
        } else {
            self.clone()
        };
        ...
        for locate in location {
            match locate {
                ...
                _ => {          // 默认情况则搜索
                    ...
                    if !sign {
                        let child_node = FileTreeNode(Rc::new(RefCell::new(FileTreeNodeRaw {
                            filename:locate.to_string(),  // 文件名
                            file_type: FileType::Directory,   // 文件数类型
                            parent: None,                     // 父节点
                            children: vec![],                 // 无需
                            cluster: 0,                       // 虚拟文件cluster指向申请到的页表内存地址 默认情况下支持一个页表
                            size: 0,                          // 文件大小
                            nlinkes: 1,                       // link数量
                        ...
                        tree_node.add(child_node.clone());
        ...
        let mut inner = tree_node.0.borrow_mut();

        // 申请页表
        let page_num = alloc()?;
        let temp_buf = get_buf_from_phys_page(page_num, 1);
        temp_buf.fill(0);
        inner.file_type = FileType::VirtFile;
        inner.cluster = PhysAddr::from(page_num).into();
