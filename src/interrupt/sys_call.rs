...
use crate::{console::puts, task::{STDOUT, STDIN, STDERR}, memory::{page_table::KERNEL_PAGE_MAPPING, addr::VirtAddr}};
...
pub fn sys_call(context: &mut Context) {
    // a7(x17) 作为调用号
    match context.x[17] {
        SYS_WRITE => sys_write(context.x[10],context.x[11],context.x[12]),
        _ => {
            info!("未识别调用号");
        }
    }
