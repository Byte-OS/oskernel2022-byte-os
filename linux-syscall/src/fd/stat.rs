use kernel::{task::{fd_table::FD_NULL}, memory::addr::UserAddr, fs::{file::{Kstat, FileType}, filetree::INode, StatFS}, runtime_err::RuntimeError};

use crate::SyscallTask;

pub fn sys_fstat(task: SyscallTask, fd: usize, buf_ptr: UserAddr<Kstat>) -> Result<(), RuntimeError> {
    debug!("sys_fstat: {}", fd);
...
pub fn sys_statfs(task: SyscallTask, _fd: usize, buf_ptr: UserAddr<StatFS>) -> Result<(), RuntimeError> {
